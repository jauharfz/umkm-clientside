...
export default function StokRow({ item, index, onEdit, onDelete }) {
    ...
    const katClass = {
        ...
        Fashion  : "ms-kat-fashion",
        Kerajinan: "ms-kat-kerajinan",
    }[item.kategori] || "ms-kat-lainnya";

    const fillClass   = isLow ? "ms-fill-low"    : isWarn ? "ms-fill-warn"   : "ms-fill-ok";
    const statusClass = isLow ? "ms-status-low"  : isWarn ? "ms-status-warn" : "ms-status-ok";
    ...
    return (
        <tr>
            ...
            {/* FOTO + NAMA */}
            <td>
                <div className="ms-td-nama-wrap">
                    {item.foto_url
                        ? <img src={item.foto_url} alt={item.nama} className="ms-td-foto" />
                        : <div className="ms-td-foto-empty">📦</div>
                    }
                    <div>
            ...
            {/* HARGA */}
            <td>
                <span className="ms-harga">Rp {Number(item.harga).toLocaleString("id-ID")}</span>
            </td>
            ...
            <td>
                <div className="ms-aksi">
                    <button className="ms-btn-edit" onClick={() => onEdit(item)}><Pencil size={12} /></button>
                    <button className="ms-btn-del"  onClick={() => onDelete(item)} title="Hapus"><Trash size={12} /></button>
                </div>
            </td>
        </tr>
    );
